-- PostgreSQL's inet::text includes /32 or /128; the UI and current-IP guard
-- need the host address without its mask.
...
BEGIN
  ...
  RETURN QUERY
    SELECT host(a.address), a.label, a.created_at, a.created_by, host(a.address) = v_current_ip
    FROM crm_private.admin_ip_allowlist a ORDER BY a.created_at, a.address;
END;
$$;

CREATE OR REPLACE FUNCTION public.crm_admin_add_ip(p_ip text, p_label text DEFAULT '')
...
BEGIN
  ...
  VALUES (auth.uid(), 'admin_ip_added', jsonb_build_object('ip', host(v_address), 'label', v_label), 'CRM network access');
END;
$$;
...
BEGIN
  ...
  END;
  v_current_ip := nullif(current_setting('request.headers', true), '')::jsonb ->> 'cf-connecting-ip';
  IF host(v_address) = v_current_ip THEN
