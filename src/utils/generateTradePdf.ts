...
async function loadBrandLogo(): Promise<string> {
  ...
  if (!response.ok) throw new Error('Unable to load the brand logo');
...
function addBrandLogo(doc: jsPDF, logo: string, x: number, y: number, height: number) {
  const logoAspectRatio = 1944 / 809;
  doc.addImage(logo, 'PNG', x, y, height * logoAspectRatio, height, 'brand-logo', 'FAST');
}
...
export async function generateTradePdf(
  ...
  addBrandLogo(doc, logo, margin, 7, 21);
  ...
  autoTable(doc, {
    ...
    didDrawPage(data) {
      ...
      doc.setFont('helvetica', 'normal');
      addBrandLogo(doc, logo, margin, pageH - 11, 6);
      doc.text('dex.vestio.ai', margin + 8, pageH - 7.5);
      doc.text(`Page ${pageNum} of ${totalPages}`, pageWidth - margin, pageH - 8, { align: 'right' });

      if (pageNum > 1) {
        addBrandLogo(doc, logo, margin, 4, 10);
      }
    },
  });
