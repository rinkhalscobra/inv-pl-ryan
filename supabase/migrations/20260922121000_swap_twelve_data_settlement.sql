...
BEGIN
  ...
  SELECT q.price INTO v_eur_usd_rate FROM public.cfd_market_quotes q
  WHERE q.symbol='EUR/USD' AND q.timestamp BETWEEN now()-interval '5 minutes' AND now()+interval '1 minute';
  IF COALESCE(v_eur_usd_rate,0)<=0 THEN RAISE EXCEPTION 'A current Twelve Data EUR/USD quote is required'; END IF;

  IF v_from_symbol = 'EUR' THEN
    v_from_price := v_eur_usd_rate;
  ELSE
    SELECT q.price_usd INTO v_from_price FROM public.crypto_market_quotes q
    WHERE q.symbol=v_from_symbol || 'USDT'
      AND q.timestamp BETWEEN now()-interval '5 minutes' AND now()+interval '1 minute';
  END IF;

  IF v_to_symbol = 'EUR' THEN
    v_to_price := v_eur_usd_rate;
  ELSE
    SELECT q.price_usd INTO v_to_price FROM public.crypto_market_quotes q
    WHERE q.symbol=v_to_symbol || 'USDT'
      AND q.timestamp BETWEEN now()-interval '5 minutes' AND now()+interval '1 minute';
  END IF;

  IF COALESCE(v_from_price, 0) <= 0 OR COALESCE(v_to_price, 0) <= 0 THEN
    RAISE EXCEPTION 'A current Twelve Data price is unavailable for this swap';
  END IF;
