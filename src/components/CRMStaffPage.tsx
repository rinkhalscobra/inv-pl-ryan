...
const input =
  "w-full rounded-lg border border-white/[0.13] bg-[#0f1520] px-3 py-2 text-sm text-white outline-none focus:border-violet-400";
const nameOf = (person: {
  first_name: string | null;
  last_name: string | null;
  email: string;
  ...
  return (
    <section className={`${panel} overflow-hidden`}>
      <div className="flex items-center justify-between border-b border-white/[0.08] px-4 py-3">
        ...
        <span className="rounded-md bg-white/[0.05] px-2 py-1 text-xs text-slate-400">
          {rows.length}
        </span>
      </div>
      {rows.length === 0 ? (
        <div className="p-6 text-sm text-slate-500">No records</div>
      ) : (
...
export default function CRMStaffPage({ role }: { role: StaffRole }) {
  ...
  const loadScope = useCallback(
    async (query = search) => {
      ...
      if (requestError) {
        ...
        setScope(emptyScope(role));
        setWorkspace(null);
      } else {
        setScope((data as StaffScope) || emptyScope(role));
        setError(null);
      }
  ...
  const filteredTeamMembers = useMemo(
    () =>
      officeFilter === "all"
        ? scope.team_members
        : scope.team_members.filter(
            (member) => (member.office_id || "unassigned") === officeFilter,
          ),
  ...
  useEffect(() => {
    setSelectedClientId((current) =>
      filteredClients.some((client) => client.id === current)
        ? current
  ...
  useEffect(() => {
    ...
    void supabase
      .rpc("crm_staff_get_client_workspace", { p_client_id: selectedClientId })
  ...
  const selectedClient = useMemo(
    () => scope.clients.find((client) => client.id === selectedClientId),
    ...
    try {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "The client dashboard could not be opened.",
      );
    } finally {
      setOpeningClientId("");
    }
  ...
  const promote = async () => {
    if (
      !selectedClient ||
      !window.confirm(
        `Promote ${nameOf(selectedClient)} to Retention? Sales access will end immediately.`,
      )
    )
      return;
    setBusy("promote");
    ...
    if (requestError) setError(requestError.message);
      ...
      setNotice(
        "Client promoted. Sales access has been removed and Admin can now assign a Retention owner.",
      );
      await loadScope();
    }
    setBusy("");
  ...
  const saveDeposit = async (event: React.FormEvent) => {
    ...
    const { error: requestError } = editingDepositId
      ? await supabase.rpc("crm_workflow_update_lead_deposit", {
          p_transaction_id: editingDepositId,
          p_amount: amount,
          p_currency: deposit.currency,
          p_reference: deposit.reference.trim(),
          p_reason: deposit.reason.trim(),
        })
      : await supabase.rpc("crm_workflow_add_lead_deposit", {
          p_client_id: selectedClient.id,
          p_amount: amount,
          p_currency: deposit.currency,
          p_reference: deposit.reference.trim(),
    ...
    if (requestError) setError(requestError.message);
      ...
      setNotice(
        editingDepositId
          ? "Deposit corrected, with the original credit reversed for audit history."
          : "Deposit added and recorded in the audit log.",
      );
      setEditingDepositId("");
      setDeposit({
        amount: "",
    ...
    <div className="min-h-screen bg-[#0d1118] px-4 py-5 text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1700px]">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-4 border-b border-white/[0.09] pb-5">
          <div className="flex items-center gap-3">
            <button
              ...
              className="rounded-lg border border-white/[0.12] p-2.5 text-slate-300 hover:text-white"
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                {role.startsWith("retention") || role === "retention"
                  ? "Retention"
                  : "Sales"}{" "}
                ·{" "}
                {isWorkspaceManager
                  ? "All Offices · workspace-wide access"
                  : `${scope.office ? `${scope.office.code} · ${scope.office.name}` : "No office"} · hierarchy-scoped access`}
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {isWorkspaceManager && (
              <AppSelect
                ...
                aria-label="Filter workspace by Office"
              >
                ...
                {scope.offices.map((office) => (
                  <option key={office.id} value={office.id}>
                    ...
                    {office.status === "inactive" ? " (inactive)" : ""}
                  </option>
                ))}
              </AppSelect>
            )}
            {(role === "workflow_manager" || role === "desk_manager") && (
              <button
                onClick={() => navigate("/crm/leads")}
                className="rounded-lg border border-violet-400/30 px-3 py-2 text-sm font-semibold text-violet-200 hover:bg-violet-500/10"
              >
                Lead inbox
              </button>
            )}
            <button
              ...
              className="flex items-center gap-2 rounded-lg border border-white/[0.12] px-3 py-2 text-sm text-slate-300 hover:text-white disabled:opacity-50"
            >
              <RefreshCw
                size={16}
                className={loadingScope ? "animate-spin" : ""}
              />
              Refresh
            </button>
          </div>
        ...
        {filteredTeamMembers.length > 0 && (
          <section className={`${panel} mb-5 p-4`}>
            <h2 className="flex items-center gap-2 font-semibold">
              <Users size={17} />
              {isWorkspaceManager ? "Workspace team" : "Your team"}
            </h2>
            <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {filteredTeamMembers.map((member) => (
                <div
                  key={member.id}
                  className="rounded-lg border border-white/[0.08] bg-black/10 p-3"
                >
                  <div className="truncate font-semibold">{nameOf(member)}</div>
                  <div className="truncate text-xs text-slate-400">
                    {member.email}
                  </div>
                  <div className="mt-2 text-xs text-violet-300">
                    {roleLabels[member.role]} ·{" "}
                    {member.office_code || "No Office"}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[310px_minmax(0,1fr)]">
          <aside
            className={`${panel} h-fit overflow-hidden xl:sticky xl:top-4`}
          >
              ...
              {loadingScope ? (
                ...
                filteredClients.map((client) => (
                  <button
                    ...
                    className={`mb-1 w-full rounded-lg border px-3 py-3 text-left ${selectedClientId === client.id ? "border-violet-400/40 bg-violet-500/10" : "border-transparent hover:bg-white/[0.04]"}`}
                  >
          ...
          <main className="min-w-0 space-y-5">
            {!selectedClientId ? (
              <div
                className={`${panel} flex min-h-[330px] items-center justify-center p-8 text-center text-slate-400`}
              >
                Select an accessible client.
              </div>
            ) : loadingWorkspace ? (
              <div
                className={`${panel} flex min-h-[330px] items-center justify-center text-slate-400`}
              >
              ...
              <>
                <section className={`${panel} p-5`}>
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        {workspace.profile.is_promoted
                          ? "Retention client"
                          : "Sales lead"}
                      </span>
                      <button
                        onClick={() => void openClient(workspace.profile.id)}
                        disabled={!!openingClientId}
                        className="flex items-center gap-2 rounded-lg bg-violet-600 px-3 py-2 text-xs font-semibold hover:bg-violet-500 disabled:opacity-50"
                      >
                        {openingClientId ? (
                  ...
                  !workspace.profile.is_promoted && (
                    <section className={`${panel} p-5`}>
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <h3 className="font-semibold">Sales controls</h3>
                          <p className="mt-1 text-xs text-slate-400">
                            Add or correct a deposit, or promote this lead.
                            Promotion immediately removes the complete Sales
                            hierarchy's access.
                          </p>
                        </div>
                        <button
                          ...
                          disabled={!!busy}
                          className="rounded-lg border border-amber-400/30 px-3 py-2 text-sm font-semibold text-amber-200 hover:bg-amber-400/10 disabled:opacity-50"
                        >
                          Promote to Retention
                        </button>
                      </div>
                      <form
                        onSubmit={saveDeposit}
                        className="mt-4 grid gap-3 md:grid-cols-[140px_1fr_1fr_1fr_auto]"
                      >
                        <AppSelect
                        ...
                        >
                          {editingDepositId ? "Save correction" : "Add deposit"}
                        </button>
                      </form>
                      {editingDepositId && (
                        <button
                        ...
                        {workspace.transactions
                          .filter(
                            (row) =>
                              ...
                              String(row.description || "").startsWith(
                                "Workflow Manager deposit",
                              ),
                          )
                          .map((row) => (
                            <div
                              key={String(row.id)}
                              className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/[0.08] px-3 py-2 text-xs"
                            >
                              <span>
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  setEditingDepositId(String(row.id));
                                  setDeposit({
                                    amount: String(row.amount || ""),
                                    currency: String(row.currency || "EUR"),
                                    reference: "",
                                    reason: "Correct CRM lead deposit",
                                  });
                                }}
                ...
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                  {[
                    [
                      ...
                      formatEur(
                        convertUsdToEur(
                          Number(workspace.balance.usdt_balance || 0),
                        ),
                      ),
                      Wallet,
                    ],
                    [
                      ...
                      new Intl.NumberFormat("en-US", {
                        style: "currency",
                        currency: "USD",
                      }).format(Number(workspace.balance.usd_balance || 0)),
                      Wallet,
                    ],
                    [
                      "BTC holdings",
                      Number(workspace.balance.btc_balance || 0).toFixed(6),
                      Wallet,
                    ],
                    ["Open positions", workspace.positions.length, TrendingUp],
                    [
                      "Robot",
                      workspace.robot.is_active ? "Active" : "Inactive",
                      Bot,
                    ],
                ...
                <div className="grid gap-5 2xl:grid-cols-2">
                  ...
                  <DataTable
                    title="Positions"
                    rows={workspace.positions || []}
                    columns={["symbol", "side", "status", "created_at"]}
                  />
                  <DataTable
                    title="Orders"
