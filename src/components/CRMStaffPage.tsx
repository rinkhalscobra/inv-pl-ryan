...
import { ArrowLeft, Bot, RefreshCw, Search, ShieldCheck, TrendingUp, Users, Wallet } from 'lucide-react';
    ...
    <div className="mx-auto max-w-[1700px]">
      ...
      <div className="grid gap-5 xl:grid-cols-[310px_minmax(0,1fr)]">
        <aside className={`${panel} h-fit overflow-hidden xl:sticky xl:top-4`}>
          ...
          <div className="max-h-[70vh] overflow-y-auto p-2">{loadingScope ? <div className="p-6 text-center text-sm text-slate-400">Loading clients...</div> : visibleClients.length === 0 ? <div className="p-6 text-center text-sm text-slate-400">No assigned clients found.</div> : visibleClients.map(client => <button key={client.id} onClick={() => setSelectedClientId(client.id)} className={`mb-1 w-full rounded-lg border px-3 py-3 text-left ${selectedClientId === client.id ? 'border-violet-400/40 bg-violet-500/10' : 'border-transparent hover:bg-white/[0.04]'}`}><div className="truncate text-sm font-semibold">{nameOf(client)}</div><div className="truncate text-xs text-slate-400">{client.email}</div>{role === 'retention' && <div className="mt-1 truncate text-xs text-violet-300">{client.assignment_type === 'retention' ? 'Direct retention client' : `Agent: ${client.agent_name || '—'}`}</div>}</button>)}</div>
        </aside>
        ...
        <main className="min-w-0 space-y-5">
          {!selectedClientId ? <div className={`${panel} flex min-h-[330px] items-center justify-center p-8 text-center text-slate-400`}>Select an assigned client to view their account.</div> : loadingWorkspace ? <div className={`${panel} flex min-h-[330px] items-center justify-center text-slate-400`}>Loading client workspace...</div> : !workspace ? <div className={`${panel} flex min-h-[330px] items-center justify-center p-8 text-center text-slate-400`}>Client workspace unavailable. Refresh to try again.</div> : <>
            <section className={`${panel} p-5`}><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-bold">{nameOf(workspace.profile)}</h2><p className="mt-1 text-sm text-slate-400">{workspace.profile.email}</p><p className="mt-1 font-mono text-xs text-slate-500">{workspace.profile.id}</p></div><div className="rounded-lg border border-white/[0.1] px-3 py-2 text-xs text-slate-300">KYC: {workspace.profile.kyc_status?.replace(/_/g, ' ') || 'unknown'}</div></div><div className="mt-4 grid gap-3 border-t border-white/[0.08] pt-4 text-sm sm:grid-cols-3"><div><div className="text-xs text-slate-500">Country</div><div>{workspace.profile.country || '—'}</div></div><div><div className="text-xs text-slate-500">Phone</div><div>{workspace.profile.phone_number || '—'}</div></div><div><div className="text-xs text-slate-500">Assigned to</div><div>{selectedClient?.assignment_type === 'retention' ? 'Direct retention' : selectedClient?.agent_name || '—'}</div></div></div></section>
