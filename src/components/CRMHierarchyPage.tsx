...
interface Person { id: string; email: string; first_name: string | null; last_name: string | null; role: CRMRole; is_promoted: boolean; office_id: string | null; office_name: string | null; office_code: string | null; created_at: string; registration_source_key: string; registration_source_name: string; registration_source_kind: string }
...
export default function CRMHierarchyPage() {
  ...
  const sourceOptions = useMemo(() => Array.from(new Map(clients.map(client => [client.registration_source_key || 'unknown', client.registration_source_name || 'Unknown'])).entries()).sort((left, right) => left[1].localeCompare(right[1])), [clients]);
  const visibleClients = useMemo(() => clients.filter(client => {
    if (normalizedSearch && !`${nameOf(client)} ${client.email} ${client.id}`.toLowerCase().includes(normalizedSearch)) return false;
    const registrationDay = client.created_at?.slice(0, 10) || '';
    if (registeredFrom && registrationDay < registeredFrom) return false;
    if (registeredTo && registrationDay > registeredTo) return false;
    if (sourceFilter !== 'all' && (client.registration_source_key || 'unknown') !== sourceFilter) return false;
    const retentionOwnerId = hierarchy.retention_client_assignments.find(item => item.client_id === client.id)?.retention_id || '';
    if (retentionFilter === 'assigned' && (!client.is_promoted || !retentionOwnerId)) return false;
    if (retentionFilter === 'unassigned' && (!client.is_promoted || retentionOwnerId)) return false;
    if (retentionFilter.startsWith('owner:') && retentionOwnerId !== retentionFilter.slice(6)) return false;
    return true;
  }), [clients, hierarchy.retention_client_assignments, normalizedSearch, registeredFrom, registeredTo, retentionFilter, sourceFilter]);
  const clearClientFilters = () => { setSearch(''); setRegisteredFrom(''); setRegisteredTo(''); setRetentionFilter('all'); setSourceFilter('all'); };
  ...
  return <div className="min-h-screen bg-[#0d1118] px-4 py-5 text-slate-100 sm:px-6 lg:px-8"><div className="mx-auto max-w-[1700px]">
    ...
    <section className={`${panel} mt-5 min-w-0 overflow-hidden`}><div className="border-b border-white/[0.08] p-3 sm:p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><h2 className="flex items-center gap-2 font-semibold"><Users size={17} className="shrink-0" />Client workspace assignment</h2><p className="mt-1 text-xs leading-5 text-slate-400">Filter by registration date, Retention assignment, source, workspace and Office.</p></div><div className="flex w-full items-center justify-between gap-3 text-xs text-slate-400 sm:w-auto sm:shrink-0 sm:justify-end"><span className="whitespace-nowrap">{visibleClients.length} of {clients.length} clients</span><button type="button" onClick={clearClientFilters} className="whitespace-nowrap rounded-lg border border-white/[0.12] px-3 py-2 text-slate-300 hover:text-white">Clear filters</button></div></div><div className="mt-4 grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-6"><label className="min-w-0 text-[11px] text-slate-400 sm:col-span-2">Search client<div className="relative mt-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={15} /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Name, email or client ID" className={`${field} min-w-0 pl-9`} /></div></label><label className="min-w-0 text-[11px] text-slate-400">Registered from<input type="date" value={registeredFrom} max={registeredTo || undefined} onChange={event => setRegisteredFrom(event.target.value)} className={`${field} mt-1 min-w-0 [color-scheme:dark]`} /></label><label className="min-w-0 text-[11px] text-slate-400">Registered to<input type="date" value={registeredTo} min={registeredFrom || undefined} onChange={event => setRegisteredTo(event.target.value)} className={`${field} mt-1 min-w-0 [color-scheme:dark]`} /></label><label className="min-w-0 text-[11px] text-slate-400 lg:col-span-2 2xl:col-span-1">Retention assignment<AppSelect value={retentionFilter} onChange={event => setRetentionFilter(event.target.value)} className={`${field} mt-1 min-w-0`}><option value="all">All clients</option><option value="assigned">Retention assigned</option><option value="unassigned">Retention unassigned</option>{retentionUsers.map(user => <option key={user.id} value={`owner:${user.id}`}>{nameOf(user)}</option>)}</AppSelect></label><label className="min-w-0 text-[11px] text-slate-400 lg:col-span-2 2xl:col-span-1">Registration source<AppSelect value={sourceFilter} onChange={event => setSourceFilter(event.target.value)} className={`${field} mt-1 min-w-0`}><option value="all">All sources</option>{sourceOptions.map(([key, name]) => <option key={key} value={key}>{name}</option>)}</AppSelect></label></div></div><div className="max-w-full overflow-x-auto overscroll-x-contain"><table className="w-full min-w-[1220px] text-left text-sm"><thead className="bg-white/[0.025] text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Client</th><th className="px-4 py-3">Registered</th><th className="px-4 py-3">Source</th><th className="px-4 py-3">Workspace</th><th className="px-4 py-3">Office</th><th className="px-4 py-3">Owner</th><th className="px-4 py-3">Controls</th></tr></thead><tbody className="divide-y divide-white/[0.06]">{visibleClients.map(client => <tr key={client.id}><td className="px-4 py-3"><div className="font-semibold">{nameOf(client)}</div><div className="text-xs text-slate-400">{client.email}</div></td><td className="whitespace-nowrap px-4 py-3 text-xs text-slate-300">{new Date(client.created_at).toLocaleDateString()}</td><td className="px-4 py-3"><div className="text-xs text-slate-200">{client.registration_source_name || 'Unknown'}</div><div className="mt-1 text-[10px] text-slate-500">{(client.registration_source_kind || 'unknown').replaceAll('_', ' ')}</div></td><td className="px-4 py-3"><span className={`rounded-md px-2 py-1 text-xs ${client.is_promoted ? 'bg-amber-500/10 text-amber-200' : 'bg-cyan-500/10 text-cyan-200'}`}>{client.is_promoted ? 'Retention · promoted' : 'Sales · unpromoted'}</span></td><td className="px-4 py-3"><AppSelect value={client.office_id || ''} disabled={!!busy} onChange={event => assignOffice(client, event.target.value)} className={field}><option value="">No office</option>{hierarchy.offices.map(office => <option key={office.id} value={office.id} disabled={office.status !== 'active'}>{office.code} · {office.name}</option>)}</AppSelect></td><td className="px-4 py-3"><AppSelect value={ownerOf(client)} disabled={!!busy} onChange={event => assignClient(client, event.target.value)} className={field}><option value="">Unassigned</option>{(client.is_promoted ? retentionUsers : agents).filter(owner => (owner.office_id || '') === (client.office_id || '')).map(owner => <option key={owner.id} value={owner.id}>{nameOf(owner)}</option>)}</AppSelect></td><td className="px-4 py-3"><div className="flex gap-2">{!client.is_promoted && <button onClick={() => promoteClient(client)} disabled={!!busy} className="rounded-lg border border-amber-400/30 px-3 py-2 text-xs font-semibold text-amber-200 hover:bg-amber-400/10 disabled:opacity-50">Promote</button>}<button onClick={() => void openClient(client)} disabled={!!openingClientId} className="rounded-lg border border-white/[0.12] p-2 text-slate-300 hover:text-white">{openingClientId === client.id ? <Loader2 size={16} className="animate-spin" /> : <ExternalLink size={16} />}</button></div></td></tr>)}{visibleClients.length === 0 && <tr><td colSpan={7} className="p-8 text-center text-slate-500">No clients match the selected filters.</td></tr>}</tbody></table></div></section>
