...
interface Owner { user_id: string; role: 'agent' | 'retention'; users: { email: string; first_name: string | null; last_name: string | null } }
interface Dashboard { leads: Lead[]; total: number; sources: Source[]; owners: Owner[] }
...
const emptyDashboard: Dashboard = { leads: [], total: 0, sources: [], owners: [] };
  ...
  const registerLead = () => {
    ...
    void run(`register-${lead.id}`, async () => {
      ...
      return data.outcome === 'existing'
        ...
        : `${lead.email} is now a client and an invitation was sent.`;
    });
  };
    ...
    <div className="mx-auto max-w-[1800px]">
      <header className="mb-5 flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-5">
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => navigate('/admin')} aria-label="Back to CRM" className={`${button} border border-white/10 text-slate-300 hover:text-white`}><ArrowLeft size={18} /></button>
          <div className="rounded-lg bg-violet-500/15 p-2.5 text-violet-300"><Users size={21} /></div>
          <div><h1 className="text-2xl font-bold">Lead inbox</h1><p className="text-sm text-slate-400">Collect affiliate leads and invite them to become clients.</p></div>
        </div>
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
        <section className={`${panel} min-w-0 overflow-hidden`}>
          <div className="flex flex-wrap items-center gap-3 border-b border-white/10 p-4">
            <div className="mr-auto"><h2 className="font-semibold">Leads</h2><p className="text-xs text-slate-400">Register a lead to send a secure client invitation.</p></div>
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[780px] text-left text-sm"><thead className="border-b border-white/10 bg-[#111723] text-xs text-slate-400"><tr><th className="px-4 py-3">Lead</th><th className="px-4 py-3">Contact</th><th className="px-4 py-3">Source</th><th className="px-4 py-3">Received</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody className="divide-y divide-white/[0.07]">
            {dashboard.leads.map(lead => <tr key={lead.id} className="hover:bg-white/[0.025]">
              ...
              <td className="px-4 py-3"><span className={`rounded-md px-2 py-1 text-xs ${lead.status === 'new' ? 'bg-violet-500/15 text-violet-200' : lead.status === 'registered' ? 'bg-emerald-500/15 text-emerald-200' : 'bg-white/5 text-slate-300'}`}>{lead.status === 'inviting' ? 'Processing' : lead.status === 'existing' ? 'Existing client' : lead.status}</span></td>
              <td className="px-4 py-3 text-right">{lead.status === 'new' ? <button type="button" onClick={() => { setSelectedLead(lead); setOwner(''); }} disabled={!!busy} className={`${button} bg-violet-600 text-white hover:bg-violet-500`}><Send size={14} />Register</button> : lead.status === 'inviting' ? <span className="text-xs text-slate-400">Invitation in progress</span> : <span className="text-xs text-slate-500">Linked</span>}</td>
            </tr>)}
      ...
      {selectedLead && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4"><section role="dialog" aria-modal="true" aria-labelledby="register-lead-title" className="w-full max-w-lg rounded-2xl border border-white/15 bg-[#171e2b] p-6 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><h2 id="register-lead-title" className="text-lg font-semibold">Register {nameOf(selectedLead)}</h2><p className="mt-1 text-sm text-slate-400">{selectedLead.email}</p></div><button type="button" onClick={() => setSelectedLead(null)} disabled={!!busy} aria-label="Close" className="text-slate-400 hover:text-white"><X size={20} /></button></div><div className="mt-5 rounded-lg border border-violet-400/20 bg-violet-500/10 p-4 text-sm text-slate-200"><ShieldCheck size={18} className="mb-2 text-violet-300" />This creates a client account and emails an invitation to set a password.</div>{error && <div role="alert" className="mt-4 rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-200">{error}</div>}<label className="mt-5 block text-xs font-medium text-slate-300">Assign client to <span className="font-normal text-slate-500">(optional)</span><AppSelect value={owner} onChange={event => setOwner(event.target.value)} className={`mt-1.5 ${input}`}><option value="">Unassigned</option><optgroup label="Agents">{dashboard.owners.filter(item => item.role === 'agent').map(item => <option key={item.user_id} value={`agent:${item.user_id}`}>{ownerName(item)}</option>)}</optgroup><optgroup label="Retention">{dashboard.owners.filter(item => item.role === 'retention').map(item => <option key={item.user_id} value={`retention:${item.user_id}`}>{ownerName(item)}</option>)}</optgroup></AppSelect></label><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => setSelectedLead(null)} disabled={!!busy} className={`${button} border border-white/10 text-slate-300`}>Cancel</button><button type="button" onClick={registerLead} disabled={!!busy} className={`${button} bg-violet-600 text-white hover:bg-violet-500`}>{busy ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />}Send invitation</button></div></section></div>}
    </div>
  </main>;
}
